...
const RENDERER_VERSION = "playwright-1.62.1-chromium-v3-r4-fit";
const INSPECTOR_VERSION = "dom-geometry-v3-r4-fit";
...
function blockHtml(block) {
  ...
  if (block.kind === "metric") return `<div class="block metric" data-block-id="${id}">${block.label ? `<span>${escapeHtml(block.label)}</span>` : ""}<strong>${escapeHtml(block.text)}</strong></div>`;
...
function htmlForPage(request, page) {
  const colors = {background: COLOR_TOKENS[request.theme.background], surface: COLOR_TOKENS[request.theme.surface], text: COLOR_TOKENS[request.theme.text], muted: COLOR_TOKENS[request.theme.muted_text], accent: COLOR_TOKENS[request.theme.accent], border: COLOR_TOKENS[request.theme.border]};
...
async function inspectPage(browser, request, pageSpec, duplicatePage) {
  return withPreparedPage(browser, request, pageSpec, async (page) => {
    const observation = await page.evaluate((spec) => {
      ...
      const tolerance = 1;
      const clipping = frame.scrollWidth > frame.clientWidth + tolerance || frame.scrollHeight > frame.clientHeight + tolerance || content.scrollWidth > content.clientWidth + tolerance || content.scrollHeight > content.clientHeight + tolerance || blockRects.some(({rect}) => rect.left < frameRect.left - tolerance || rect.right > frameRect.right + tolerance || rect.top < frameRect.top - tolerance || rect.bottom > frameRect.bottom + tolerance);
      let overlap = false;
...
async function readJson(req) { let size=0; const chunks=[]; for await (const chunk of req) { size+=chunk.length; if(size>MAX_BODY_BYTES)fail("renderer request body too large",413); chunks.push(chunk); } if(!chunks.length)fail("renderer request body required"); try{return JSON.parse(Buffer.concat(chunks).toString("utf8"));}catch{fail("renderer request must be valid JSON");} }
function json(res,status,payload){const body=Buffer.from(JSON.stringify(payload));res.writeHead(status,{"content-type":"application/json; charset=utf-8","content-length":body.length,"cache-control":"no-store","x-content-type-options":"nosniff"});res.end(body);}

const server=http.createServer(async(req,res)=>{try{
  if(req.method==="GET"&&req.url==="/health")return json(res,200,{status:"ok",renderer_name:RENDERER_NAME,renderer_version:RENDERER_VERSION,visual_qa:INSPECTOR_VERSION});
  if(req.method!=="POST"||!["/render","/qa"].includes(req.url))return json(res,404,{detail:"not found"});
  if(!(req.headers["content-type"]||"").toLowerCase().startsWith("application/json"))fail("application/json required",415);
  const request=validateRequest(await readJson(req)); const browser=await getBrowser();
  if(req.url==="/render"){const pages=[];for(const pageSpec of request.pages)pages.push(await renderPage(browser,request,pageSpec));return json(res,200,{render_id:request.render_id,render_input_digest:request.render_input_digest,renderer_name:RENDERER_NAME,renderer_version:RENDERER_VERSION,pages});}
  const signatures=request.pages.map(semanticPageSignature); const observations=[];
  for(const pageSpec of request.pages){const signature=signatures[pageSpec.page_index];const duplicatePage=signatures.findIndex((candidate)=>candidate===signature)!==pageSpec.page_index;observations.push(await inspectPage(browser,request,pageSpec,duplicatePage));}
  return json(res,200,{render_id:request.render_id,render_input_digest:request.render_input_digest,inspector_version:INSPECTOR_VERSION,observations});
}catch(error){console.error("[renderer]",error?.message||error);return json(res,Number.isInteger(error?.status)?error.status:500,{detail:error?.status?error.message:"render failed"});}});
server.listen(PORT,"0.0.0.0",()=>console.log(`[renderer] listening on ${PORT}`));
async function shutdown(){server.close();if(browserPromise){try{await(await browserPromise).close();}catch{}}process.exit(0);}process.on("SIGTERM",shutdown);process.on("SIGINT",shutdown);
