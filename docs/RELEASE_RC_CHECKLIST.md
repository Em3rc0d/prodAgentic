...
- the complete npm dependency graph passes the HIGH-severity audit gate;
...
The certification commit must be a normal repository commit whose parent already contains the final generated Python and npm locks. CI results from an earlier or bot-generated dependency head are not release evidence.
