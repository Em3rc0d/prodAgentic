...
Status: **IMPLEMENTED — canonical identity is bound by the PR head and workflow receipt.**
...
The candidate SHA is intentionally **not embedded in this tracked file**. Doing so would make the commit identity self-referential. The authoritative candidate identity is the exact PR head recorded by GitHub and independently echoed by `S10-CERT` as `checkout_sha` + `pr_head_sha`. Any content mutation creates a new candidate and resets consensus.
...
**CANDIDATE IDENTITY: EXTERNALIZED TO EXACT PR HEAD / WORKFLOW RECEIPT**  
**S10 CERTIFIED/CLOSED: NO — until exact-SHA pre- and post-merge consensus**
