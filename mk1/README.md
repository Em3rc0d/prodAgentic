...
Current S4 product certificate:

```text
6a0a653d615e7fa2d1d63bc41b6b265b19646202
```

Earlier certificate boundaries remain independently valid. Documentation-only descendants may advance `main` without redefining the exact product-code certificate.
...
The certified implementation reaches the typed `VisualSpecV1` boundary. S5 is the first slice authorized to turn that visual intent into owned render bytes.

## Certified S4 handoff
...
Critical editorial copy is referenced from accepted `ContentSpecV1`; it is not freely regenerated by the visual layer.

S4 keeps the run in `VISUAL_PLANNING`. Renderer execution, AssetStore, pixel QA and the state transition to `RENDERING` remain S5+ authority.
...
- [`test/evidence/S4/CERTIFICATION.md`](test/evidence/S4/CERTIFICATION.md)
...
17. Product certificates, receipt heads, documentation descendants and operator-machine acceptance are distinct evidence boundaries.
...
S5 must start from the final S4-certified `main` descendant and may not reinterpret `VisualSpecV1` or critical copy authority without a new accepted contract.
