...
type ScheduleRequestCapture = {
  scheduled_for?: unknown;
  timezone_context?: unknown;
};
...
test("S10 Calendar exposes Week Month Queue, scheduling intent and reconciliation without blind retry", async ({ page }) => {
  ...
  const scheduleCapture: { value?: ScheduleRequestCapture } = {};
  let reconciles = 0;
  await page.route("**/api/approvals/approval-s10-ready/schedules", async (route) => {
    scheduleCapture.value = route.request().postDataJSON() as ScheduleRequestCapture;
  ...
  await expect.poll(() => scheduleCapture.value).toBeDefined();
  expect(typeof scheduleCapture.value?.scheduled_for).toBe("string");
  expect(typeof scheduleCapture.value?.timezone_context).toBe("string");
  expect(String(scheduleCapture.value?.timezone_context).length).toBeGreaterThan(0);
